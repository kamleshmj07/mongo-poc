...
async function run(req, res, next, endpoint, description, joinKey, pipeline) {
  try {
    ...
    const data = await getDB().collection('onboards').aggregate(pagedPipeline).toArray();
...
// GET /api/split/onboards/summary
// One row per onboard document — header fields only, no service arrays
router.get('/summary', (req, res, next) => run(req, res, next,
  '/api/split/onboards/summary',
  'One row per onboard request — employee, manager, requester, company, workflow status',
  'docId',
  [{
    $project: {
      _id: 0,
...
// GET /api/split/onboards/services
// One row per service task — fields[] pivoted as field_<key> columns
router.get('/services', (req, res, next) => run(req, res, next,
  '/api/split/onboards/services',
...
// GET /api/split/onboards/workflow-steps
// One row per workflow step (workflow.Data[]) — join to summary on docId
router.get('/workflow-steps', (req, res, next) => run(req, res, next,
  '/api/split/onboards/workflow-steps',
